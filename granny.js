...
const tour = (friends, fTowns, distTable) => {
  const distances = distTable.filter((elem, index) => index % 2 !== 0)
  distances.splice(friends.length)
  let total = distances[0] + distances[distances.length-1];

  for (let i = 0; i < distances.length-1; i++) {
    total = total + Math.sqrt((distances[i+1] ** 2 - distances[i] ** 2))
  }
